...
## Side task: player-specific study

A second, structurally different side task alongside video-to-cards above: instead of theory (masters/online statistics) or coaching-video extraction, this builds a **scouting profile of one specific opponent** the repo owner plays against regularly online, from that opponent's own game archive. Opened 2026-09-09 with `players/guigui/Guigui_White_System.md` (Guigui4378's White repertoire); a Black-repertoire companion for the same player is a planned follow-up, and this methodology is meant to generalize to future players/profiles without re-deriving it from scratch — read this whole section first if picking that up.

**The `players/<name>/` convention:**
- `players/<name>/<name>-white_<year>.pgn` and `players/<name>/<name>-black_<year>.pgn` hold the raw PGN archive, one file per color the tracked player played (a mixed archive works too — pass the same file for both args to the tree builder; it matches games to the tracked player by the PGN's own White/Black header, not by which file they came from).
- `players/<name>/<Name>_White_System.md` (and, eventually, `_Black_...md`) holds the authored profile itself.
- A dumped tree JSON (`players/<name>/<name>_tree.json`, from `player_tree.py --dump-json`) is a reasonable intermediate artifact to keep alongside the PGNs if it's cheap to regenerate and useful for a future session to re-query without re-parsing — not required to commit, but harmless if it is.

**The toolchain**, both promoted to `tools/` this batch and exercised end-to-end for the first time on Guigui's archive:
```
python tools/player_tree.py <PlayerName> <white.pgn> <black.pgn> --max-plies 20 --dump-json <out>.json
python tools/player_report.py <out>.json white 16 0.15 2 20   # args: max_depth(plies) min_share opp_top_k min_games
python tools/player_report.py <out>.json black 16 0.15 2 20
```
`player_tree.py` replays every game (via the same `fen.py`/`apply_san.py` primitives every other card uses) into a frequency tree keyed by a **normalized FEN** (board+turn+castling+ep, halfmove/fullmove counters dropped) — this is what makes transposing move orders collapse onto one shared tree node, and it's the direct mechanism behind "this player transposes between different ECO codes without realizing it": two different move orders reaching the same position show up as the exact same node with the exact same continuation stats. `player_report.py` walks the tree and prints it indented: at the tracked player's own turns, every reply carrying at least `min_share` of that node's traffic (their real, repeatable choices, always at least the top one); at the opponent's turns, the top `opp_top_k` replies regardless of share (keeps the tree from exploding while still showing the real alternatives a reader would face); `min_games` is a floor to stop descending into noise.

**Why this needs its own formatting rules, not the standard card template** (`start.md`'s skeleton still governs board diagrams/FENs/Lichess links — only the stats and diagram mechanics differ):
- **No `<!-- lichess-stats:start/end -->` markers.** Those are reserved for `tools/update_stats.py`, which queries live masters/online frequency — meaningless for one specific opponent's own practice, and running the updater repo-wide without excluding `players/` could silently overwrite hand-authored personal-frequency tables with garbage (there's no matching FEN context for it to find). Author frequency tables by hand instead, headed something unambiguous like **"Guigui's own games (n=NNN)"** — never presented in a way that could be mistaken for the repo's usual masters/online stats convention.
- **No Overview-mermaid shape classification** (rectangle/subroutine/rhombus/stadium/hexagon, `start.md`'s "Content diagram" section) — those thresholds are calibrated for a masters-vs-online percentage comparison that doesn't exist here. Skip the shape system entirely (plain prose/tables, or unclassified rectangles only if a diagram is used at all) and skip `tools/check_diagram.py` for files under `players/` specifically — it isn't built to understand a personal-frequency data model and will misfire trying to match "moves" against tables that don't follow its expected shape. (Still run `check_diagram.py` **repo-wide** after any `players/` edit, same as any other change, to confirm nothing else broke — just don't expect it to validate the `players/` file itself.)
- Everything else stays identical to a normal card: FENs derived via `apply_san.py` (never hand-typed), board diagrams via the same backscattering.de/Lichess convention, every eval a live Stockfish `cloud-eval` query — the "sourcing & verification discipline" rules above apply in full.
- A genuinely new pattern worth carrying forward: cross-check whether the *live opening tag* exists for a player's own most-repeated deep nodes (`tools/explore.py`'s `opening` field) before writing content — Guigui's own system turned out to hit `opening: None` at several of its most-played nodes even in the masters database (i.e. genuinely uncharted, not just "a rare line"), which is itself worth stating plainly as a finding, not something to force a name onto.

**Status**: Guigui4378's White repertoire is done (`players/guigui/Guigui_White_System.md`) — a personally-consistent "1.d4, then 2.e3 almost regardless of Black's setup" system, `opening: None` at its own deepest repeated nodes, with a cross-reference added to `d4_openings/D00_Queens_Pawn_Game.md` (whose existing Stonewall-Attack 2.e3 mention is a *different* idea, not to be conflated). His Black repertoire (`players/guigui/Guigui4378-black_2025.pgn`, same toolchain, `black` instead of `white` in `player_report.py`) is the open follow-up — the tree JSON dump already covers both colors in one file if regenerated, so it's a `player_report.py ... black ...` invocation away, not a re-run of `player_tree.py`.
